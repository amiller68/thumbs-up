...
async fn main() {
    ...
    use thumbs_up_http::app::{AppState, Config};
    ...
    thumbs_up_http::register_panic_logger();
    thumbs_up_http::report_version();
    ...
    let (graceful_waiter, shutdown_rx) = thumbs_up_http::graceful_shutdown_blocker();
    let mut handles = Vec::new();

    let server = thumbs_up_http::server(*config.log_level(), state, shutdown_rx).await;
    handles.push(server);
